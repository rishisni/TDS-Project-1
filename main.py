...
#         response = requests.get(users_url, headers=HEADERS, params={**USER_PARAMS, 'page': page, 'per_page': 100}).json()
        
#         # Stop if no items found or if we've reached the rate limit
#         if 'items' not in response:
#             print("No more user data found or check API rate limit.")
#             break
...
#         # Go to the next page
#         page += 1
#         if len(users_data) >= 465:  # Stop if we've reached the expected count
#             break
...
def fetch_users_in_boston():
    ...
    users_data = []
    page = 1
    max_pages = 5  # You can adjust this to control the number of pages you fetch (GitHub limits results to 1000 items for search)

    while True:
        params = {**USER_PARAMS, 'per_page': 100, 'page': page}
        response = requests.get(users_url, headers=HEADERS, params=params).json()

        if 'items' not in response:
            print("No user data found or rate limit exceeded.")
            break
        ...
        print(f"Fetched page {page}, total users so far: {len(users_data)}")

        if len(response['items']) < 100 or page >= max_pages:  # Stop if fewer than 100 users were returned or max pages reached
            break

        page += 1

    print(f"Fetched a total of {len(users_data)} users.")
    return users_data
